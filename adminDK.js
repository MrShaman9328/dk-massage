...
// очевиден (как в РАЙ). Внутри логики (сортировка, ключи schedule/bookings)
// везде остаётся ISO 'YYYY-MM-DD' — эта функция только для вывода.
  ...
  for (let i = 0; i < SCHEDULE_DAYS_AHEAD; i++) {
    ...
    dayEl.innerHTML =
      '<div class="schedule-day-head">' +
        '<span class="day-label">' + formatDateRu(iso) + '</span>' +
  ...
  sorted.forEach(function (b) {
    ...
    card.innerHTML =
      '<div class="a-card-top">' +
        '<span class="a-card-date">' + escapeHtml(formatDateRu(b.date)) + ', ' + escapeHtml(b.start) + '–' + escapeHtml(b.end) + '</span>' +
    ...
    card.querySelector('.btn-delete').addEventListener('click', function () {
      if (!confirm('Удалить запись ' + b.clientName + ' на ' + formatDateRu(b.date) + '?')) return;
...
function renderHistory() {
  ...
  sorted.forEach(function (h) {
    ...
    card.innerHTML =
      '<div class="a-card-top"><span class="a-card-title">' + escapeHtml(h.serviceName) + '</span><span class="a-card-meta">' + escapeHtml(formatDateRu(h.date)) + ', ' + escapeHtml(h.start) + '</span></div>' +
...
function renderClients() {
  ...
  sorted.forEach(function (c) {
    ...
    card.innerHTML =
      ...
      '<p class="a-card-line">Потрачено: ' + Number(c.totalSpent || 0).toLocaleString('ru-RU') + ' ₽ · последний визит ' + escapeHtml(c.lastVisit ? formatDateRu(c.lastVisit) : '—') + '</p>' +
...
function renderRequests() {
  ...
  sorted.forEach(function (req) {
    ...
    card.className = 'a-card';
    card.innerHTML =
      '<div class="a-card-top"><span class="a-card-title">' + escapeHtml(req.name) + '</span><span class="a-card-meta">' + (req.createdAt ? escapeHtml(formatDateRu(req.createdAt.slice(0, 10))) : '') + '</span></div>' +
